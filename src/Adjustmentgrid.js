  ...
  const AdjustmnetHdrDelete = async () => {
    try {
      const response = await fetch(`${config.apiBaseUrl}/Adjustmnetdelhdr`, {
        ...
        body: JSON.stringify({ company_code: sessionStorage.getItem('selectedCompanyCode'), Location_Code: sessionStorage.getItem('selectedLocationCode'), transaction_no: transaction_no })
      });
      if (response.ok) {
        return true;
      } else {
