...
function Sales() {
  ...
  const [CountDrop, setCountDrop] = useState([]);
    ...
    fetch(`${config.apiBaseUrl}/getItem`, {
      ...
      .then((data) => {
        ...
        const defaultProduct =
          data.find((item) => item.attributedetails_name === "Product") || data[0];
        ...
        if (defaultProduct) {
          ...
          // Run fetchCodes immediately without checking invoicetype
          if (defaultProduct.attributedetails_name === "Product") {
  ...
  const handleChangeCount = (selectedOption) => {
  ...
  const filteredOptionCount = CountDrop.map((option) => ({
    value: option.attributedetails_name,
    label: option.attributedetails_name,
  }));

  useEffect(() => {
    fetch(`${config.apiBaseUrl}/getCount`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        company_code: sessionStorage.getItem("selectedCompanyCode"),
  ...
  const ItemSalesAmountCalculation = async (params, extraValues) => {
    try {
      const response = await fetch(`${config.apiBaseUrl}/getSalesItemAmountCalculation`, {
        ...
        body: JSON.stringify({
          ...
          bill_qty: Number(extraValues.bill_qty),
          discount: params.data.discount,
          item_amt: params.data.purchaseAmt,
          ...
          Weight: Number(extraValues.Weight),
          measurements: extraValues.measurements,
          company_code: sessionStorage.getItem("selectedCompanyCode")
        })
      });
      ...
      const updatedRowData = rowData.map(row => {
        if (row.itemCode === params.data.itemCode && row.serialNumber === params.data.serialNumber) {
          ...
          if (matchedItem) {
            return {
              ...row,
              unitWeight: matchedItem.unitWeight,
      ...
      searchData.forEach(item => {
        const existWrong = updatedRowDataTaxCopy.find(row => row.ItemSNO === item.ItemSNO && row.Item_code !== item.Item_code);
        if (existWrong) {
          updatedRowDataTaxCopy = updatedRowDataTaxCopy.filter(x => x !== existWrong);
        }

        const existSame = updatedRowDataTaxCopy.find(row =>
          row.ItemSNO === item.ItemSNO &&
          row.Item_code === item.Item_code &&
          row.TaxType === item.TaxType
        );

        if (existSame) {
          existSame.TaxPercentage = item.TaxPercentage ?? 0;
          existSame.TaxAmount = parseFloat(item.TaxAmount ?? 0).toFixed(2);
        } else {
          updatedRowDataTaxCopy.push({
      ...
      if (hasSalesQty) {
        ...
        await TotalAmountCalculation(totalTaxAmounts, totalItemAmounts);
      }
    } catch (error) {
  ...
  const handleItemCode = async (selectedOption) => {
    ...
    const Itemcode = selectedOption ? selectedOption.value : null;
    ...
    try {
      ...
      if (response.ok) {
        ...
        const newRows = searchData.map((matchedItem, index) => {
          const serial = lastSerialNumber + index + 1;
          return {
            serialNumber: serial,
        ...
        // ✅ Set the last added row for Quantity calculation
        setLastAddedRow(newRows[newRows.length - 1]);
          ...
          <div className="shadow-lg p-1 bg-body-tertiary rounded  mb-2 mt-2">
            <div class="d-flex justify-content-between">
              ...
              <div className="col-md-1 form-group mb-2">
                <div class="exp-form-floating " title="Select the Screen">
                  <Select
                    id="returnType"
                    className="exp-input-field"
                    placeholder=""
