...
function ItemInput({ open, handleClose, itemCode, itemImage }) {
  ...
  const logoSrc = selectedImage
    ? selectedImage
    : itemImage
      ? `data:image/jpeg;base64,${typeof itemImage === 'object' ? arrayBufferToBase64(itemImage.data) : itemImage}`
      : null;
  ...
  const handleInsert = async () => {
    ...
    try {
      ...
      const response = await fetch(`${config.apiBaseUrl}/UpdateItemImage`, {
        method: "POST",
        body: formData,
      });
