...
function Purchase() {
  ...
  const BarcodeScanner = ({ onClose, onScan }) => {
    ...
    const startScan = async () => {
      try {
        ...
        // 👇 IMPORTANT: Delay fixes Samsung WebView race condition
        setTimeout(async () => {
  ...
  const columnDefs = [
    ...
    },
    {
      ...
      cellRenderer: (params) => {
        ...
        const showIcons = isWideEnough && context?.buttonsVisible;
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      if (headerData && detailData && taxData) {
        ...
        console.log("Failed to fetch some data");
        toast.error("Transaction No Does Not Exist");
      }
    } catch (error) {
      console.error("Error executing API calls:", error);
      toast.error('Error inserting data: ' + error.message);
  ...
  const handleTransactionDateChange = (e) => {
    ...
    if (date >= financialYearStart && date <= financialYearEnd) {
      ...
      toast.warning('Transaction date must be between April 1st, 2025 and March 31st, 2026.');
    }
  };
  ...
  const sampleSiteData = [
    { site_id: "ST-001", site_name: "Main Corporate Warehouse", location: "Building A", status: "Active" },
  ...
  const handleSiteSelect = (site) => {
    ...
    console.log("Selected Site Payload:", site);
    // You can call your backend API or trigger AG Grid updates using site.site_id here
  };
          ...
          <div>
            <div className="shadow-lg p-1 bg-body-tertiary rounded  mb-2 mt-2">
              <div class="d-flex justify-content-between">
                ...
                <div className="d-flex justify-content-end purbut me-3">
                  ...
                  <printbutton className={`purbut uds-btn ${showSiteDropdown ? 'active-uds' : ''}`} title="UDS" onClick={handleUdsToggle}>
                    {/* <i className="fa-solid fa-database"></i>  */}
                    UDS
                  </printbutton>

                  {/* ==========================================================================
                    TOPBAR NOTIFICATION-STYLE SITE SELECTION CONTAINER
                  ========================================================================== */}
                  {showSiteDropdown && (
                    <div className="uds-site-dropdown-card me-3 animate-site-fade-in shadow-lg">

                      {/* Header block modeled after Topbar alert notification setup */}
                      <div className="site-dropdown-header d-flex justify-content-between align-items-center">
                        <div>
                          <h6 className="m-0 text-white"><i className="fa-solid fa-layer-group me-2 text-warning"></i>Select Active Site</h6>
                          <small className="text-white-50">Available operations nodes</small>
                        </div>
                        <span className="badge bg-primary px-2 py-1">{sampleSiteData.length} Sites</span>
                      </div>

                      {/* Body Content Container holding list maps */}
                      <div className="site-dropdown-body custom-scrollbar-uds">
                        {sampleSiteData.map((site) => (
                          <div
                            key={site.site_id}
                            className={`site-item-row d-flex align-items-center justify-content-between ${selectedSite?.site_id === site.site_id ? 'active-selected-row' : ''}`}
                            onClick={() => handleSiteSelect(site)}
                          >
                            <div className="d-flex align-items-center gap-3">
                              {/* Left Context Marker Icon Wrapper */}
                              <div className="site-avatar-icon bg-soft-warning text-warning">
                                <i className="fa-solid fa-location-dot"></i>
                              </div>

                              {/* Central Details Typography Meta */}
                              <div className="site-meta-details">
                                <div className="d-flex align-items-center gap-2">
                                  <span className="site-id-text">{site.site_id}</span>
                                  <span className={`site-status-dot ${site.status.toLowerCase()}`}></span>
                                </div>
                                <p className="site-name-text m-0 text-truncate" style={{ paddingLeft: '0px' }}>{site.site_name}</p>
                              </div>
                            </div>

                            {/* Right Dynamic Interaction Visual Flag */}
                            <div className="site-action-zone">
                              {selectedSite?.site_id === site.site_id ? (
                                <span className="current-indicator"><i className="fa-solid fa-circle-check text-success fs-5"></i></span>
                              ) : (
                                <span className="select-arrow-action"><i className="fa-solid fa-chevron-right text-white-50"></i></span>
                              )}
                            </div>
                          </div>
                        ))}
                  ...
                  <printbutton className="purbut" title="Settings" onClick={navigateToPurchaseSettings}>
                ...
                <div class="d-flex justify-content-between">
                  ...
                  <div class="dropdown mt-1 ms-5" style={{ paddingLeft: 0 }}>
                    ...
                    <ul class="dropdown-menu menu">
                      ...
                      <li className="iconbutton d-flex justify-content-center text-primary uds-mobile-li">
                        <icon className={`icon uds-mobile-btn ${showSiteDropdown ? 'active-uds' : ''}`}  onClick={handleUdsToggle} style={{ background: 'none', border: 'none' }}>
                          <i className="fa-solid fa-database"></i>
                        </icon>
                      </li>
                      ...
                      <li className="iconbutton d-flex justify-content-center text-dark">
                        <icon className="icon" onClick={navigateToPurchaseSettings}>
                          <i className="fa-solid fa-gear"></i>
                        </icon>
                      </li>
                    </ul>
            ...
            <div className="shadow-lg p-1 bg-body-tertiary rounded  pt-3 pb-4" align="left">
              <div className="row ms-3 me-3">
                ...
                <div className="col-md-3 form-group mb-2">
                  <label htmlFor="party_code" className={`${error && !transactionNumber ? 'red' : ''}`}>Transaction No {showAsterisk && <span className="text-danger">*</span>}</label>
