...
const PurchaseOrderAnalysis = () => {
  ...
  const [columnDefs] = useState([
    ...
    {
      headerName: "Ship to Vendor Name",
  ...
  const fetchPOData = async () => {
    ...
    try {
      ...
      const response = await fetch(`${config.apiBaseUrl}/getPOperiod`, {
        method: "POST",
        headers: {
      ...
      if (response.ok) {
        ...
        const newRows = fetchedData.map((matchedItem) => ({
          ...
          vendor_addr_1: matchedItem.vendor_addr_1,
          vendor_addr_2: matchedItem.vendor_addr_2,
          vendor_addr_3: matchedItem.vendor_addr_3,
          vendor_addr_4: matchedItem.vendor_addr_4,
          ...
          ship_to_state: matchedItem.ship_to_state,
          ship_to_country: matchedItem.ship_to_country,
          ship_to_contact_number: matchedItem.ship_to_contact_number,
          ship_to_contact_person: matchedItem.ship_to_contact_person,
          total_purchase_amount: matchedItem.total_purchase_amount,
          total_tax_amount: matchedItem.total_tax_amount,
          total_rounded_off: matchedItem.total_rounded_off,
          total_amount: matchedItem.total_amount,
        }));
        
        const totalAmount = newRows.reduce((sum, row) => sum + row.total_amount, 0);
        const totalPurchase = newRows.reduce((sum, row) => sum + row.total_purchase_amount, 0);
        const totalRoundOff = newRows.reduce((sum, row) => sum + row.total_rounded_off, 0);
        const totalTax = newRows.reduce((sum, row) => sum + row.total_tax_amount, 0);
        ...
        const totalRow = {
          ...
          ship_to_contact_person: "Total",
          total_purchase_amount: totalPurchase,
          total_tax_amount: totalTax,
          total_rounded_off: totalRoundOff,
  ...
  //       "Bill to Vendor Name": row.vendor_name,
  //       "Bill to Vendor Address": `
  //         ${row.vendor_addr_1 || ''},
  //         ${row.vendor_addr_2 || ''},  
  //         ${row.vendor_addr_3 || ''},  
  //         ${row.vendor_addr_4 || ''},<br>
  //         ${row.state || ''},<br>
  //         ${row.country || ''}`,
  //       "Bill to Contact Person": row.contact_person,
  //       "Bill to Mobile No": row.contact_number,
  //       "Ship to Vendor Name": row.ShipTo_customer_name,
  //       "Ship to Vendor Address": `
  //         ${row.ShipTo_customer_addr_1 || ''},
  //         ${row.ShipTo_customer_addr_2 || ''},  
  //         ${row.ShipTo_customer_addr_3 || ''},  
  //         ${row.ShipTo_customer_addr_4 || ''},<br>
  //         ${row.ship_to_state || ''},<br>
  //         ${row.ship_to_country || ''}`,
  //       "Ship to Contact Person": row.ship_to_contact_person,
  //       "Ship to Mobile No": row.ship_to_contact_number,
  //       "Total": row.total_purchase_amount,
  //       "Total Tax": row.total_tax_amount,
  //       "Round Off": row.total_rounded_off,
  //       "Grand Total": row.total_amount,
  //     };
  //   });

  //   const reportWindow = window.open("", "_blank");
  //   reportWindow.document.write("<html><head><title>Purchase Order Analysis</title>");
  ...
  //   reportWindow.document.write("<h1><u>Purchase Order Analysis</u></h1>");
  ...
  const transformRowData = (data) => {
    return data.map(row => ({
      "Transaction Date": row.Entry_date,
      "Transaction No": row.transaction_no,
      "Bill to Vendor Name": row.vendor_name.toString(),
      "Bill to Address 1": row.vendor_addr_1.toString(),
      "Bill to Address 2": row.vendor_addr_2.toString(),
      "Bill to Address 3": row.vendor_addr_3.toString(),
      "Bill to Address 4": row.vendor_addr_4.toString(),
      "Bill to State": row.state.toString(),
      "Bill to Country": row.country.toString(),
      "Bill to Mobile No": row.contact_number.toString(),
      "Bill to Contact Person": row.contact_person.toString(),
      "Ship to Vendor Name": row.ShipTo_customer_name.toString(),
      ...
      "Ship to State": row.ship_to_state.toString(),
      "Ship to Country": row.ship_to_country.toString(),
      "Ship to Mobile no": row.ship_to_contact_number.toString(),
      "Ship to Contact Person": row.ship_to_contact_person.toString(),
      "Total": row.total_purchase_amount.toString(),
      "Total Tax": row.total_tax_amount.toString(),
      "Round Off": row.total_rounded_off.toString(),
  ...
  const handleExportToExcel = () => {
    ...
    XLSX.writeFile(workbook, 'Purchase_Order_Analysis.xlsx');
  };
    ...
    <div className="container-fluid Topnav-screen">
      ...
      <div className="shadow-lg p-1 bg-body-tertiary rounded mb-2">
        <div className="d-flex justify-content-between ">
          <h1 className='purbut mt-3'>Purchase Order Analysis</h1>
          <div className="mobileview">
            <div className="d-flex justify-content-between ">
              <div className="d-flex justify-content-start ">
                <h1 className='h1'>Purchase Order Analysis</h1>
              </div>
        ...
        <div className="row ms-4 mt-3 mb-3 me-4">
          ...
          <div className="col-12 col-md-3">
            ...
            <input
              ...
              value={transactionNo}
              onKeyDown={(e) => e.key === "Enter" && fetchPOData()}
              onChange={(e) => setTransactionNo(e.target.value)}
            />
          </div>
          <div className="col-12 col-md-3">
            <label className="form-label">Bill to Vendor Name</label>
            <input
              id="wcode"
              className="form-control exp-input-field"
              placeholder=""
              title='Please Enter Bill to Vendor Name'
              value={customerName}
              onKeyDown={(e) => e.key === "Enter" && fetchPOData()}
          ...
          <div className="col-12 col-md-3">
            ...
            <input
              ...
              className="form-control exp-input-field"
              placeholder=""
              title='Please Enter the Ship to Customer Name'
          ...
          <div className="col-12 col-md-3 mt-2">
            ...
            <input
              ...
              className="form-control exp-input-field"
              placeholder=""
               title='Please Enter the Ship to Customer Address'
          ...
          <div className="col-md-1">
              <div class="exp-form-floating">
                <div class=" d-flex justify-content-center mt-5">
                    <icon className="popups-btn fs-6 p-3" onClick={fetchPOData} required title="Search">
